...
impl Resampler {
    ...
    pub fn run(&mut self, l_audio: &[i16], r_audio: &[i16]) -> Result<Vec<i16>> {
        ensure!(l_audio.len() == r_audio.len(), "Audio buffer length mismatch!");

        let mut in_frame = frame::Audio::new(Sample::I16(Type::Packed), l_audio.len(), ChannelLayout::STEREO);
        in_frame.set_rate(32000);

        let packed_samples: Vec<(i16, i16)> = zip(l_audio.iter().cloned(), r_audio.iter().cloned()).collect();
        in_frame.plane_mut::<(i16, i16)>(0).copy_from_slice(&packed_samples);

        let out_samples = (l_audio.len() as i64).rescale_with(
            Rational::from(self.rsp_ctx.output().rate as f64),
            Rational::from(32000.0),
            Rounding::Up
        );
